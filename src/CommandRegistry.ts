...
export class CommandRegistry {
  ...
  unregisterCommandGroup(commandGroup: CommandGroup) {
    this.commandGroups = this.commandGroups.filter(group => group !== commandGroup);
  }
  ...
  private get filteredCommands() {
    return this.filteredCommandGroups.reduce((commands, group) => commands.concat(group.filteredCommands), []);
  }

  get filteredCommandGroups() {
    return this.availableCommandGroups
      .filter(commandGroup => commandGroup.filteredCommands.length > 0);
  }
}
